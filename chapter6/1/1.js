...
NVMCClient.drawLeg = function (gl, side, shaderToUse) {
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cylinder, shaderToUse, [0.24, 0.40, 0.63, 1.0], [0, 0, 0, 1]);
	stack.pop();
};
...
NVMCClient.drawJoint = function (gl, shaderToUse) {
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.sphere, shaderToUse, [0.8, 0.2, 0.2, 1.0], [0, 0, 0, 1]);
	stack.pop();
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cylinder, shaderToUse, [0.95, 0.79, 0.61, 1.0], [0, 0, 0, 1]);
	stack.pop();
};
...
NVMCClient.drawBody = function (gl, shaderToUse) {
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cube, shaderToUse, [0.8, 0.2, 0.2, 1.0], [0, 0, 0, 1]);
	stack.pop();
};
...
NVMCClient.drawScene = function (gl, num) {
	...
	if (this.currentCamera != 3) {
		...
		this.drawCar(gl, this.lambertianSingleColorMultiLightShader);
		stack.pop();
	} else {
