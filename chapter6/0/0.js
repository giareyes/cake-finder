...
NVMCClient.drawHair = function (gl, shaderToUse) {
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cylinder, shaderToUse, [0.8, 0.2, 0.2, 1.0]);
	stack.pop();
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.sphere, shaderToUse, [0.55, 0.35, 0.11, 1.0]);
	stack.pop();
...
NVMCClient.drawHead = function (gl, shaderToUse) {
	var stack = this.stack;
...
NVMCClient.drawLeg = function (gl, side, shaderToUse) {
	var stack = this.stack;
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cylinder, shaderToUse, [0.24, 0.40, 0.63, 1.0]);
	stack.pop();
};
...
NVMCClient.drawJoint = function (gl, shaderToUse) {
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.sphere, shaderToUse, [0.8, 0.2, 0.2, 1.0]);
	stack.pop();
	...
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cylinder, shaderToUse, [0.95, 0.79, 0.61, 1.0]);
	stack.pop();
};

NVMCClient.drawArm = function (gl, side, shaderToUse) {
	var stack = this.stack;
...
NVMCClient.drawBody = function (gl, shaderToUse) {
	var stack = this.stack;
	...
	this.drawArm(gl, "right", shaderToUse);
	this.drawArm(gl, "left", shaderToUse);
	this.drawLeg(gl, "right", shaderToUse);
	this.drawLeg(gl, "left", shaderToUse);
	gl.uniformMatrix4fv(shaderToUse.uModelViewMatrixLocation, false, stack.matrix);
	var InvT = SglMat4.inverse(this.stack.matrix);
	InvT = SglMat4.transpose(InvT);
	gl.uniformMatrix3fv(shaderToUse.uViewSpaceNormalMatrixLocation, false, SglMat4.to33(InvT));
	this.drawObject(gl, this.cube, shaderToUse, [0.8, 0.2, 0.2, 1.0]);
	stack.pop();
};

NVMCClient.drawCar = function (gl, shaderToUse) {//line 142,
	var stack = this.stack;
	...
	this.drawHead(gl, shaderToUse);
	this.drawBody(gl, shaderToUse);
	stack.pop();
};
...
NVMCClient.drawScene = function (gl, num) {
	...
	if (this.currentCamera != 3) {
		...
		this.drawCar(gl, this.lambertianSingleColorShader);
		stack.pop();
	} else {
